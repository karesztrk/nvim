...
local function create_floating_window(opts)
    ...
    local win_config = {
        title = opts.title or "Floating terminal",
        relative = "editor",
        width = width,
        height = height,
        col = col,
...
local toggle_floating_terminal = function(opts)
    if not vim.api.nvim_win_is_valid(state.floating.win) then
        ...
        state.floating = create_floating_window { title = cmd, buf = state.floating.buf }
        if vim.bo[state.floating.buf].buftype ~= "terminal" then
            ...
            vim.api.nvim_set_option_value('winblend', 15, { win = state.floating.win })

            -- Set transparent background for the window
            vim.api.nvim_set_option_value('winhighlight',
                'Normal:FloatingTermNormal,FloatBorder:FloatingTermBorder', { win = state.floating.win })
